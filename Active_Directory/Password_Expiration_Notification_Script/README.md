...
This script sends automated email reminders to Active Directory users before their domain password expires. It calculates the expiry date using the resultant fine-grained password policy when available, otherwise it uses the default domain password policy.

## Original reference

This solution is based on the Microsoft Tech Community article:
...
The version in this repo adapts that idea to use Microsoft Graph `sendMail`, a DPAPI-protected credential file, selected OU scope, CSV logging, and an administrator run report.
...
4. Exclude users where password never expires or password is already expired.
5. Calculate password expiry from fine-grained password policy or default domain password policy.
...
The report includes the run date/time, server name, testing mode, expiry window, total users processed, emails sent, skipped users, missing email count, send failures, and the log file path.
